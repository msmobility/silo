 ...
 *
 * @author    Carlee A Clymer
 * @version   1.0, 9/4/2013
 */
public class CSVSquareTableMatrixReaderCC extends MatrixReader {

    static final Logger logger = Logger.getLogger(CSVSquareTableMatrixReaderCC.class);

    File myFile;

    public CSVSquareTableMatrixReaderCC(File myFile) {
        super();
        this.myFile = myFile;
    }
    ...
    public Matrix readMatrix(String name) throws MatrixException {
        ...
        try {
            inStream = new BufferedReader( new FileReader(myFile) );
        } catch (FileNotFoundException e) {
            throw new RuntimeException("Can't open file "+myFile);
        }
        float[][] values;
        int[] zones;
        try {
        ...
        Matrix m = new Matrix(name,null,values);
