...
/* - Allocate a buffer for the file records
    ...
    */
    ...
    for (b=list->Mnames,i=1; i<=nMat1;i++) b += 1 + sprintf(b,"%s",matName[i-1]);
...
int tppWrite (char *fileName, double *MatrixOP, int nzones, int ntables) {
	...
	for(i=0;i<ntables;++i){
		sprintf(buffer,"%i M%i",i+1,i+1);  
		names[i]=buffer;
	}
	return tppWriteWithNames (fileName, MatrixOP, nzones, ntables,names);
