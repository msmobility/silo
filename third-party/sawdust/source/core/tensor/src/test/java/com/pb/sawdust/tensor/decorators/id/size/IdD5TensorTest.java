 ...
 */
public abstract class IdD5TensorTest<T,I> extends IdTensorTest<T,I> {
    protected IdD5Tensor<T,I> dTensor;

    @Before
    @SuppressWarnings("unchecked") //cast is ok
    public void beforeTest() {
        super.beforeTest();
        dTensor = (IdD5Tensor<T,I>) tensor;
    }
    ...
    public void testGetValueByIdD() {
        ...
        assertEquals(data.getValue(index), dTensor.getValueById(ids[0],ids[1],ids[2],ids[3],ids[4]));
    }
    ...
    public void testGetValueByIdBadIdD() {
        ...
        dTensor.getValueById(ids[0],ids[1],ids[2],ids[3],ids[4]);
    }
    ...
    public void testSetValueByIdD() {
        ...
        I[] ids = getIdsFromIndices(index);
        dTensor.setValueById(value,ids[0],ids[1],ids[2],ids[3],ids[4]);
    ...
    public void testSetValueByIdBadIdD() {
        ...
        I[] ids = getIdsFromIndices(index);
        ids[random.nextInt(index.length)] = (I) new Object();
        dTensor.setValueById(getRandomElement(),ids[0],ids[1],ids[2],ids[3],ids[4]);
    }

    @Test
    public void testIteratorType() {
        for (Tensor<T> t : tensor)
            assertTrue(t instanceof IdD4Tensor);
    }

    @Test
    public void TestSizeSpecific() {
        assertEquals(5,dTensor.size());
    }
    ...
    public void testGetValueD() {
        ...
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        assertEquals(data.getValue(index),dTensor.getValue(index[0],index[1],index[2],index[3],index[4]));
    }
    ...
    public void testGetValueInvalidIndexTooBigD() {
        ...
        dTensor.getValue(index[0],index[1],index[2],index[3],index[4]);
    }
    ...
    public void testSetValueD() {
        ...
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        T value = getRandomElement();
        dTensor.setValue(value,index[0],index[1],index[2],index[3],index[4]);
    ...
    public void testSetValueInvalidIndexTooBigD() {
        ...
        int randomIndex = random.nextInt(dimensions.length);
        index[randomIndex] = dimensions[randomIndex];
        dTensor.setValue(getRandomElement(),index[0],index[1],index[2],index[3],index[4]);
    }
