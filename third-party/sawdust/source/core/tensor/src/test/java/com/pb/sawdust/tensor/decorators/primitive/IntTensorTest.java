 ...
 */
public abstract class IntTensorTest extends TensorTest<Integer> {
    protected IntTensor pTensor;
    protected IntTypeSafeArray pData;

    protected JavaType getJavaType() {
        return JavaType.INT;
    }

    abstract protected IntTypeSafeArray getData();
    abstract protected IntTensor getTensor(TypeSafeArray<Integer> data);

    @Before
    public void beforeTest() {
        super.beforeTest();
        pTensor = (IntTensor) tensor;
        pData = (IntTypeSafeArray) data;
    }
    ...
    public void testSetTensorValuesPrimitiveArray() {
        IntTypeSafeArray newValues = TypeSafeArrayFactory.intTypeSafeArray(dimensions);
        int[] index = new int[dimensions.length];
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        int value = getRandomElement();
    ...
    public void testIteratorType() {
        for (Tensor<Integer> t : tensor)
            assertTrue(t instanceof IntTensor);
    }
}
