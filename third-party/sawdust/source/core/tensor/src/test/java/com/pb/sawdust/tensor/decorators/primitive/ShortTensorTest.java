 ...
 */
public abstract class ShortTensorTest extends TensorTest<Short> {
    protected ShortTensor pTensor;
    protected ShortTypeSafeArray pData;

    protected JavaType getJavaType() {
        return JavaType.SHORT;
    }

    abstract protected ShortTypeSafeArray getData();
    abstract protected ShortTensor getTensor(TypeSafeArray<Short> data);

    @Before
    public void beforeTest() {
        super.beforeTest();
        pTensor = (ShortTensor) tensor;
        pData = (ShortTypeSafeArray) data;
    }
    ...
    public void testSetTensorValuesPrimitiveArray() {
        ShortTypeSafeArray newValues = TypeSafeArrayFactory.shortTypeSafeArray(dimensions);
        int[] index = new int[dimensions.length];
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        short value = getRandomElement();
    ...
    public void testIteratorType() {
        for (Tensor<Short> t : tensor)
            assertTrue(t instanceof ShortTensor);
    }
}
