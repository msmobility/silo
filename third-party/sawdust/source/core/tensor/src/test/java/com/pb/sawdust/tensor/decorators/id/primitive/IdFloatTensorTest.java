 ...
 */
public abstract class IdFloatTensorTest<I>  extends IdTensorTest<Float,I> {
    protected IdFloatTensor<I> pTensor;
    protected FloatTypeSafeArray pData;

    @Before
    @SuppressWarnings("unchecked") //cast is ok
    public void beforeTest() {
        super.beforeTest();
        pTensor = (IdFloatTensor<I>) tensor;
        pData = (FloatTypeSafeArray) data;
    }
    ...
    public void testIteratorType() {
        for (Tensor<Float> t : tensor)
            assertTrue(t instanceof IdFloatTensor);
    }
    ...
    public void testSetTensorValuesPrimitiveArray() {
        FloatTypeSafeArray newValues = TypeSafeArrayFactory.floatTypeSafeArray(dimensions);
        int[] index = new int[dimensions.length];
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        float value = getRandomElement();
