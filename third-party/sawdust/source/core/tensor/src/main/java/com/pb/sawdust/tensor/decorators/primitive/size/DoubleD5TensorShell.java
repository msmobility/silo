...
/**
 * The {@code DoubleD5TensorShell} class is a wrapper which sets a 5-dimensional {@code DoubleTensor} as a {@code D5Tensor} (or,
 * more specifically, a {@code DoubleD5Tensor}).
 *
 * @author crf <br/>
 *         Started: Sat Oct 25 21:35:12 2008
 *         Revised: Dec 14, 2009 12:35:29 PM
 */
public class DoubleD5TensorShell extends D5TensorShell<Double> implements DoubleD5Tensor {
     ...
     */
    public DoubleD5TensorShell(DoubleTensor tensor) {
        super(tensor);
        this.tensor = tensor;
    }
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code DoubleTensor.getCell(d0index,d1index,d2index,d3index,d4index)}, 
    ...
    public double getCell(int ... indices) {
        if (indices.length != 5)
            throw new IllegalArgumentException("DoubleD5Tensor is 5 dimension in size, getValue passed with " + indices.length + " indices.");
    ...
    public Double getValue(int d0index, int d1index, int d2index, int d3index, int d4index) {
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code DoubleTensor.setCell(double,d0index,d1index,d2index,d3index,d4index)}, 
     * and should be overridden if any efficiencies over that method can be made.
     */
    ...
    public void setValue(Double value, int d0index, int d1index, int d2index, int d3index, int d4index) {
