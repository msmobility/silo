...
/**
 * The {@code ShortD6TensorShell} class is a wrapper which sets a 6-dimensional {@code ShortTensor} as a {@code D6Tensor} (or,
 * more specifically, a {@code ShortD6Tensor}).
 *
 * @author crf <br/>
 *         Started: Sat Oct 25 21:35:12 2008
 *         Revised: Dec 14, 2009 12:35:29 PM
 */
public class ShortD6TensorShell extends D6TensorShell<Short> implements ShortD6Tensor {
     ...
     */
    public ShortD6TensorShell(ShortTensor tensor) {
        super(tensor);
        this.tensor = tensor;
    }
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code ShortTensor.getCell(d0index,d1index,d2index,d3index,d4index,d5index)}, 
    ...
    public short getCell(int ... indices) {
        if (indices.length != 6)
            throw new IllegalArgumentException("ShortD6Tensor is 6 dimension in size, getValue passed with " + indices.length + " indices.");
    ...
    public Short getValue(int d0index, int d1index, int d2index, int d3index, int d4index, int d5index) {
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code ShortTensor.setCell(short,d0index,d1index,d2index,d3index,d4index,d5index)}, 
     * and should be overridden if any efficiencies over that method can be made.
     */
    ...
    public void setValue(Short value, int d0index, int d1index, int d2index, int d3index, int d4index, int d5index) {
