...
/**
 * The {@code FloatD6TensorShell} class is a wrapper which sets a 6-dimensional {@code FloatTensor} as a {@code D6Tensor} (or,
 * more specifically, a {@code FloatD6Tensor}).
 *
 * @author crf <br/>
 *         Started: Sat Oct 25 21:35:12 2008
 *         Revised: Dec 14, 2009 12:35:30 PM
 */
public class FloatD6TensorShell extends D6TensorShell<Float> implements FloatD6Tensor {
     ...
     */
    public FloatD6TensorShell(FloatTensor tensor) {
        super(tensor);
        this.tensor = tensor;
    }
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code FloatTensor.getCell(d0index,d1index,d2index,d3index,d4index,d5index)}, 
    ...
    public float getCell(int ... indices) {
        if (indices.length != 6)
            throw new IllegalArgumentException("FloatD6Tensor is 6 dimension in size, getValue passed with " + indices.length + " indices.");
    ...
    public Float getValue(int d0index, int d1index, int d2index, int d3index, int d4index, int d5index) {
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code FloatTensor.setCell(float,d0index,d1index,d2index,d3index,d4index,d5index)}, 
     * and should be overridden if any efficiencies over that method can be made.
     */
    ...
    public void setValue(Float value, int d0index, int d1index, int d2index, int d3index, int d4index, int d5index) {
