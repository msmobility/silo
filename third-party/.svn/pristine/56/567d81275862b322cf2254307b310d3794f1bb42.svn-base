package com.pb.sawdust.tensor.decorators.primitive.size;

import com.pb.sawdust.tensor.decorators.primitive.CharTensor;
import com.pb.sawdust.tensor.decorators.id.primitive.IdCharTensor;
import com.pb.sawdust.tensor.index.Index;

/**
 * The {@code CharD0TensorShell} class is a wrapper which sets a rank 0 (scalar) {@code CharTensor} as a
 * {@code D0Tensor} (or, more specifically, a {@code CharD0Tensor}).
 *
 * @author crf <br/>
 *         Started: Jun 25, 2009 2:16:12 PM
 */
public class CharD0TensorShell extends AbstractCharD0Tensor {
    private final CharTensor tensor;

    /**
     * Constructor specifying tensor to wrap.
     *
     * @param tensor
     *        The tensor to wrap.
     *
     * @throws IllegalArgumentException if {@code tensor.size() != 0}.
     */
    public CharD0TensorShell(CharTensor tensor) {
        if (tensor.size() != 0)
            throw new IllegalArgumentException("Wrapped tensor must be of rank 0.");
        this.tensor = tensor;
    }

    public char getCell() {
        return tensor.getCell();
    }

    public void setCell(char value) {
        tensor.setCell(value);
    }

    public <I> IdCharTensor<I> getReferenceTensor(Index<I> index) {
        return tensor.getReferenceTensor(index);
    }
}
