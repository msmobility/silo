 ...
 */
public abstract class IdD6TensorTest<T,I> extends IdTensorTest<T,I> {
    protected IdD6Tensor<T,I> dTensor;

    @Before
    @SuppressWarnings("unchecked") //cast is ok
    public void beforeTest() {
        super.beforeTest();
        dTensor = (IdD6Tensor<T,I>) tensor;
    }
    ...
    public void testGetValueByIdD() {
        ...
        assertEquals(data.getValue(index), dTensor.getValueById(ids[0],ids[1],ids[2],ids[3],ids[4],ids[5]));
    }
    ...
    public void testGetValueByIdBadIdD() {
        ...
        dTensor.getValueById(ids[0],ids[1],ids[2],ids[3],ids[4],ids[5]);
    }
    ...
    public void testSetValueByIdD() {
        ...
        I[] ids = getIdsFromIndices(index);
        dTensor.setValueById(value,ids[0],ids[1],ids[2],ids[3],ids[4],ids[5]);
    ...
    public void testSetValueByIdBadIdD() {
        ...
        I[] ids = getIdsFromIndices(index);
        ids[random.nextInt(index.length)] = (I) new Object();
        dTensor.setValueById(getRandomElement(),ids[0],ids[1],ids[2],ids[3],ids[4],ids[5]);
    }

    @Test
    public void testIteratorType() {
        for (Tensor<T> t : tensor)
            assertTrue(t instanceof IdD5Tensor);
    }

    @Test
    public void TestSizeSpecific() {
        assertEquals(6,dTensor.size());
    }
    ...
    public void testGetValueD() {
        ...
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        assertEquals(data.getValue(index),dTensor.getValue(index[0],index[1],index[2],index[3],index[4],index[5]));
    }
    ...
    public void testGetValueInvalidIndexTooBigD() {
        ...
        dTensor.getValue(index[0],index[1],index[2],index[3],index[4],index[5]);
    }
    ...
    public void testSetValueD() {
        ...
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        T value = getRandomElement();
        dTensor.setValue(value,index[0],index[1],index[2],index[3],index[4],index[5]);
    ...
    public void testSetValueInvalidIndexTooBigD() {
        ...
        int randomIndex = random.nextInt(dimensions.length);
        index[randomIndex] = dimensions[randomIndex];
        dTensor.setValue(getRandomElement(),index[0],index[1],index[2],index[3],index[4],index[5]);
    }
