...
/**
 * The {@code ConcurrentD5TensorShell} class provides a wrapper for implementations of the {@code D5Tensor} interface
 ...
 */
public class ConcurrentD5TensorShell<T> extends ComposableTensor<T> implements D5Tensor<T> {
    private final D5Tensor<T> tensor;
    
    /**
     * The {@code ConcurrentD5TensorLocks} instance holding the concurrency policy used when locking this tensor.
     */
    protected final ConcurrentD5TensorLocks locks;
     ...
     */
    public ConcurrentD5TensorShell(D5Tensor<T> tensor, ConcurrentD5TensorLocks locks) {
        this.tensor = tensor;
        this.locks = locks;
    }
    
    /**
     * The {@code ConcurrentD5TensorLocks} interface provides methods through which locks protecting concurrent 5-dimensional tensors 
     ...
     */
    public static interface ConcurrentD5TensorLocks {
         ...
         */
        public abstract Lock getReadLock(int d0index, int d1index, int d2index, int d3index, int d4index);
         ...
         */
        public abstract Lock getWriteLock(int d0index, int d1index, int d2index, int d3index, int d4index);
     ...
     *        The index of remaining dimension 3 of the tensor element.
         ...
         */
        public abstract Lock getRowReadLock(int dimension, int remainingD0index, int remainingD1index, int remainingD2index, int remainingD3index);
     ...
     *        The index of remaining dimension 3 of the tensor element.
         ...
         */
        public abstract Lock getRowWriteLock(int dimension, int remainingD0index, int remainingD1index, int remainingD2index, int remainingD3index);
    ...
    public T getValue(int ... indices) {
        ...
        lock.lock();
        try {
            return tensor.getValue(indices[0],indices[1],indices[2],indices[3],indices[4]);
        } finally {
            lock.unlock();
        }
    }
    ...
    public void setValue(T value, int ... indices) {
        ...
        lock.lock();
        try {
            tensor.setValue(value,indices[0],indices[1],indices[2],indices[3],indices[4]);
        } finally {
            lock.unlock();
        }
    }
