 ...
 */
public abstract class CharTensorTest extends TensorTest<Character> {
    protected CharTensor pTensor;
    protected CharTypeSafeArray pData;

    protected JavaType getJavaType() {
        return JavaType.CHAR;
    }

    abstract protected CharTypeSafeArray getData();
    abstract protected CharTensor getTensor(TypeSafeArray<Character> data);

    @Before
    public void beforeTest() {
        super.beforeTest();
        pTensor = (CharTensor) tensor;
        pData = (CharTypeSafeArray) data;
    }
    ...
    public void testSetTensorValuesPrimitiveArray() {
        CharTypeSafeArray newValues = TypeSafeArrayFactory.charTypeSafeArray(dimensions);
        int[] index = new int[dimensions.length];
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        char value = getRandomElement();
    ...
    public void testIteratorType() {
        for (Tensor<Character> t : tensor)
            assertTrue(t instanceof CharTensor);
    }
}
