 ...
 */
public abstract class BooleanD4TensorTest extends BooleanTensorTest {
    BooleanD4Tensor pdTensor;

    abstract protected BooleanD4Tensor getTensor(TypeSafeArray<Boolean> data);
    
    @Before
    public void beforeTest() {
        super.beforeTest();
        pdTensor = (BooleanD4Tensor) tensor;
    }
    ...
    public void testGetCellD() {
        ...
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        assertAlmostEquals(pData.get(index),pdTensor.getCell(index[0],index[1],index[2],index[3]));
    }
    ...
    public void testGetCellInvalidIndexTooBigD() {
        ...
        int randomIndex = random.nextInt(dimensions.length);
        index[randomIndex] = dimensions[randomIndex];
        pdTensor.getCell(index[0],index[1],index[2],index[3]);
    }
    ...
    public void testSetCellD() {
        ...
        for (int i : range(index.length))
            index[i] = random.nextInt(dimensions[i]);
        boolean value = getRandomElement();
        pdTensor.setCell(value,index[0],index[1],index[2],index[3]);
    ...
    public void testSetCellInvalidIndexTooBigD() {
        ...
        int randomIndex = random.nextInt(dimensions.length);
        index[randomIndex] = dimensions[randomIndex];
        pdTensor.setCell(getRandomElement(),index[0],index[1],index[2],index[3]);
    }
    ...
    public void testIteratorType() {
        for (Tensor<Boolean> t : tensor)
            assertTrue(t instanceof BooleanD3Tensor);
    }
}
