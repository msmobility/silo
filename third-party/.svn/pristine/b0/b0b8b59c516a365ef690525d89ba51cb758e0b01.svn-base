...
/**
 * The {@code ConcurrentD9TensorShell} class provides a wrapper for implementations of the {@code D9Tensor} interface
 ...
 */
public class ConcurrentD9TensorShell<T> extends ComposableTensor<T> implements D9Tensor<T> {
    private final D9Tensor<T> tensor;
    
    /**
     * The {@code ConcurrentD9TensorLocks} instance holding the concurrency policy used when locking this tensor.
     */
    protected final ConcurrentD9TensorLocks locks;
     ...
     */
    public ConcurrentD9TensorShell(D9Tensor<T> tensor, ConcurrentD9TensorLocks locks) {
        this.tensor = tensor;
        this.locks = locks;
    }
    
    /**
     * The {@code ConcurrentD9TensorLocks} interface provides methods through which locks protecting concurrent 9-dimensional tensors 
     ...
     */
    public static interface ConcurrentD9TensorLocks {
         ...
         */
        public abstract Lock getReadLock(int d0index, int d1index, int d2index, int d3index, int d4index, int d5index, int d6index, int d7index, int d8index);
         ...
         */
        public abstract Lock getWriteLock(int d0index, int d1index, int d2index, int d3index, int d4index, int d5index, int d6index, int d7index, int d8index);
     ...
     *
     * @param remainingD7index
     *        The index of remaining dimension 7 of the tensor element.
         *
         * @return the read lock for the specified tensor row.
         */
        public abstract Lock getRowReadLock(int dimension, int remainingD0index, int remainingD1index, int remainingD2index, int remainingD3index, int remainingD4index, int remainingD5index, int remainingD6index, int remainingD7index);
     ...
     *        The index of remaining dimension 7 of the tensor element.
         ...
         */
        public abstract Lock getRowWriteLock(int dimension, int remainingD0index, int remainingD1index, int remainingD2index, int remainingD3index, int remainingD4index, int remainingD5index, int remainingD6index, int remainingD7index);
    ...
    public T getValue(int ... indices) {
        ...
        lock.lock();
        try {
            return tensor.getValue(indices[0],indices[1],indices[2],indices[3],indices[4],indices[5],indices[6],indices[7],indices[8]);
        } finally {
            lock.unlock();
        }
    }
    ...
    public void setValue(T value, int ... indices) {
        ...
        lock.lock();
        try {
            tensor.setValue(value,indices[0],indices[1],indices[2],indices[3],indices[4],indices[5],indices[6],indices[7],indices[8]);
        } finally {
            lock.unlock();
        }
    }
