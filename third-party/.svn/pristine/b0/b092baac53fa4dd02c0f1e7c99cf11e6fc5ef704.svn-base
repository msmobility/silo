...
/**
 * The {@code BooleanD6TensorShell} class is a wrapper which sets a 6-dimensional {@code BooleanTensor} as a {@code D6Tensor} (or,
 * more specifically, a {@code BooleanD6Tensor}).
 *
 * @author crf <br/>
 *         Started: Sat Oct 25 21:35:12 2008
 *         Revised: Dec 14, 2009 12:35:30 PM
 */
public class BooleanD6TensorShell extends D6TensorShell<Boolean> implements BooleanD6Tensor {
     ...
     */
    public BooleanD6TensorShell(BooleanTensor tensor) {
        super(tensor);
        this.tensor = tensor;
    }
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code BooleanTensor.getCell(d0index,d1index,d2index,d3index,d4index,d5index)}, 
    ...
    public boolean getCell(int ... indices) {
        if (indices.length != 6)
            throw new IllegalArgumentException("BooleanD6Tensor is 6 dimension in size, getValue passed with " + indices.length + " indices.");
    ...
    public Boolean getValue(int d0index, int d1index, int d2index, int d3index, int d4index, int d5index) {
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code BooleanTensor.setCell(boolean,d0index,d1index,d2index,d3index,d4index,d5index)}, 
     * and should be overridden if any efficiencies over that method can be made.
     */
    ...
    public void setValue(Boolean value, int d0index, int d1index, int d2index, int d3index, int d4index, int d5index) {
