...
/**
 * The {@code ConcurrentD4TensorShell} class provides a wrapper for implementations of the {@code D4Tensor} interface
 ...
 */
public class ConcurrentD4TensorShell<T> extends ComposableTensor<T> implements D4Tensor<T> {
    private final D4Tensor<T> tensor;
    
    /**
     * The {@code ConcurrentD4TensorLocks} instance holding the concurrency policy used when locking this tensor.
     */
    protected final ConcurrentD4TensorLocks locks;
     ...
     */
    public ConcurrentD4TensorShell(D4Tensor<T> tensor, ConcurrentD4TensorLocks locks) {
        this.tensor = tensor;
        this.locks = locks;
    }
    
    /**
     * The {@code ConcurrentD4TensorLocks} interface provides methods through which locks protecting concurrent 4-dimensional tensors 
     ...
     */
    public static interface ConcurrentD4TensorLocks {
         ...
         */
        public abstract Lock getReadLock(int d0index, int d1index, int d2index, int d3index);
         ...
         */
        public abstract Lock getWriteLock(int d0index, int d1index, int d2index, int d3index);
     ...
     *        The index of remaining dimension 2 of the tensor element.
         ...
         */
        public abstract Lock getRowReadLock(int dimension, int remainingD0index, int remainingD1index, int remainingD2index);
     ...
     *        The index of remaining dimension 2 of the tensor element.
         ...
         */
        public abstract Lock getRowWriteLock(int dimension, int remainingD0index, int remainingD1index, int remainingD2index);
    ...
    public T getValue(int ... indices) {
        ...
        lock.lock();
        try {
            return tensor.getValue(indices[0],indices[1],indices[2],indices[3]);
        } finally {
            lock.unlock();
        }
    }
    ...
    public void setValue(T value, int ... indices) {
        ...
        lock.lock();
        try {
            tensor.setValue(value,indices[0],indices[1],indices[2],indices[3]);
        } finally {
            lock.unlock();
        }
    }
