...
/**
 * The {@code LongD7TensorShell} class is a wrapper which sets a 7-dimensional {@code LongTensor} as a {@code D7Tensor} (or,
 * more specifically, a {@code LongD7Tensor}).
 *
 * @author crf <br/>
 *         Started: Sat Oct 25 21:35:12 2008
 *         Revised: Dec 14, 2009 12:35:31 PM
 */
public class LongD7TensorShell extends D7TensorShell<Long> implements LongD7Tensor {
     ...
     */
    public LongD7TensorShell(LongTensor tensor) {
        super(tensor);
        this.tensor = tensor;
    }
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code LongTensor.getCell(d0index,d1index,d2index,d3index,d4index,d5index,d6index)}, 
    ...
    public long getCell(int ... indices) {
        if (indices.length != 7)
            throw new IllegalArgumentException("LongD7Tensor is 7 dimension in size, getValue passed with " + indices.length + " indices.");
    ...
    public Long getValue(int d0index, int d1index, int d2index, int d3index, int d4index, int d5index, int d6index) {
     ...
     * {@inheritDoc}
     *
     * This method just calls {@code LongTensor.setCell(long,d0index,d1index,d2index,d3index,d4index,d5index,d6index)}, 
     * and should be overridden if any efficiencies over that method can be made.
     */
    ...
    public void setValue(Long value, int d0index, int d1index, int d2index, int d3index, int d4index, int d5index, int d6index) {
