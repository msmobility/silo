...
/**
 * The {@code CharTensor} interface specifies a tensor which holds {@code char} values. Though the tensor methods using {@code Character}
 * values exist and are valid, new methods defined in this interface using {@code char} values should be used in preference. Likewise, implementations
 * should store values as {@code char}s for (potential) performance reasons.
 *
 * @author crf <br/>
 *         Started: Oct 18, 2008 7:47:40 PM
 *         Revised: Dec 14, 2009 12:35:34 PM
 */
public interface CharTensor extends Tensor<Character> {
    /**
     ...
     */
    char getCell(int ... indices);
    
    /**
     * Set the cell at the specified location. This method should be more efficient than {@code setValue(Character,int[])}.
     *
     * @param value
     ...
     *                                   equal to the size of its corresponding dimension.
     */
    void setCell(char value, int ... indices);
     ...
     */
    CharTypeSafeArray getTensorValues(Class<Character> type);
    ...
    **/
    CharTypeSafeArray getTensorValues();
    
    <I> IdCharTensor<I> getReferenceTensor(Index<I> index);
    
    /**
     * {@inheritDoc}
     * 
     * The tensors this iterator loops over are guaranteed to be {@code CharTensor} tensors.
     */
