 ...
 **/
public class ConstructionOverwriteMCRImpl extends AbstractModel implements ConstructionOverwrite {

    private final static Logger logger = LogManager.getLogger(ConstructionOverwriteMCRImpl.class);
    ...
    public ConstructionOverwriteMCRImpl(DataContainer dataContainer, DwellingFactory factory, Properties properties, Random random) {
        ...
        for (Integer[] data: list) {
            ...
            if (traceOverwriteDwellings) {
                traceFile.println(ddId + "," + zoneId + "," +  dataContainer.getRealEstateDataManager().getDwellingTypes().getTypes().get(dto).toString() + "," + size + "," +
                        quality + "," + price  + "," + restrictions + "," + year);
            }
