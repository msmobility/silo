...
public class RunAggregateAccidentModel {
    ...
    public static void main(String[] args) {
        ...
        Set<Integer> targetOsmIds = Set.of(1111480, 2219688);

        System.out.println("Calculating and printing probabilities for target osmIds...");

        for (OsmLink osmLink : osmLinks) {
            if (!targetOsmIds.contains(osmLink.osmId)) continue;

            double totalLength = osmLink.getNetworkLinks().stream().mapToDouble(Link::getLength).sum();
            if (totalLength == 0) {
                System.out.println("Skipping osmId " + osmLink.osmId + " due to zero length.");
                continue;
            }

            for (int hour = 0; hour < 24; hour++) {
                double rawProb = casualtyRateCalculationOsmMCR.calculateProbability(osmLink, hour);
                double adjustedProb = rawProb/5;  // downscale 5-year risk to 1-year

                for (Link link : osmLink.getNetworkLinks()) {
                    double linkLength = link.getLength();
                    double share = linkLength / totalLength;
                    double rawLinkProb = rawProb * share;
                    double adjustedLinkProb = adjustedProb * share;

                    System.out.printf("osmId: %d, linkId: %s, hour: %d, rawProb: %.8f, adjustedProb: %.8f%n",
                            osmLink.osmId,
                            link.getId(),
                            hour,
                            rawLinkProb,
...
// Code that was outputting correct risks by osmID:
...
//                scenario, 1.);
//
//        System.out.println("Calculating pedestrian severe/fatal risk for selected OSM links...");
//
//        Set<Integer> targetOsmIds = Set.of(834267, 834268, 834269, 834637, 930354, 930356, 930713, 930715, 930978, 931013);
//
//        for (OsmLink osmLink : osmLinks) {
//            if (targetOsmIds.contains(osmLink.osmId)) {
//                for (int hour = 0; hour < 24; hour++) {
//                    try {
//                        double utility = casualtyRateCalculationOsmMCR.calculateUtility(osmLink, hour);
//                        double probZeroCrashRaw = 1.0 / (1.0 + Math.exp(-utility));
//                        double probZeroCrashAdjusted =  probZeroCrashRaw/5; // Apply downscaling
//
//                        System.out.printf("OSM ID: %d | Hour: %02d | Utility: %.4f | Raw Prob: %.5f | Adjusted Risk: %.5f%n",
//                                osmLink.osmId, hour, utility, probZeroCrashRaw, probZeroCrashAdjusted);
//                    } catch (Exception e) {
//                        System.err.printf("Error calculating risk for OSM ID %d at Hour %02d: %s%n",
//                                osmLink.osmId, hour, e.getMessage());
//                    }
//                }
//            }
//        }
...
//    }
//
//    private static void distributeRiskToLinks(OsmLink osmLink, int hour, double risk) {
//        double totalLength = osmLink.getNetworkLinks().stream()
//                .mapToDouble(Link::getLength)
//                .sum();
//
//        for (Link link : osmLink.getNetworkLinks()) {
//            double linkLength = link.getLength();
//            double proportion = totalLength > 0 ? linkLength / totalLength : 0.0;
//            double distributedRisk = risk * proportion;
//
//            System.out.printf("    --> Link ID: %s | Length: %.2f | Share: %.3f | Risk: %.5f%n",
//                    link.getId(), linkLength, proportion, distributedRisk);
//
//            // Optional: Save distributed risk per link/hour somewhere
//            // E.g., you could build a Map<Id<Link>, double[24]> to accumulate per-hour risk
//        }
//    }
//
//
