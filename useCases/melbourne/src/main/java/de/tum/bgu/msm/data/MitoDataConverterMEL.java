...
import uk.cam.mrc.phm.jobTypes.MelbourneJobType;

import java.util.Map;

public class MitoDataConverterMEL implements MitoDataConverter {

    private final static Logger logger = LogManager.getLogger(MitoDataConverterMEL.class);
    ...
    private void convertZones(DataSet dataSet, DataContainer dataContainer) {
        for (Zone siloZone : dataContainer.getGeoData().getZones().values()) {
            //TODO: check area type, urban rural? where the area type is used in the SILO model
            MitoZone zone = new MitoZone(siloZone.getZoneId(), null);
            zone.setAreaTypeR(((ZoneMEL)siloZone).getAreaTypeRType());
        ...
        for (Household siloHousehold : dataContainer.getHouseholdDataManager().getHouseholds()) {
            ...
            if (siloHousehold.getPersons().values().stream().anyMatch(p -> p.getAge() >= 18)) {
                ...
                for (Person person : siloHousehold.getPersons().values()) {
                    MitoPerson mitoPerson = convertToMitoPp((PersonHealthMEL) person, household, dataSet, dataContainer);
    ...
    private MitoPerson convertToMitoPp(PersonHealthMEL person, MitoHousehold household, DataSet dataSet, DataContainer dataContainer) {
    ...
    private void fillMitoZoneEmployees(DataSet dataSet, DataContainer dataContainer) {
        ...
        for (Job jj : dataContainer.getJobDataManager().getJobs()) {
            ...
            try {
                de.tum.bgu.msm.data.jobTypes.JobType mitoJobType = MelbourneJobType.valueOf(type);
                zone.addEmployeeForType(mitoJobType);
            } catch (IllegalArgumentException e) {
                logger.warn("Job type " + type + " not defined for MITO implementation: Melbourne");
            }
        }
    }
}
