 ...
 **/
public class EducationModelMEL extends AbstractModel implements EducationModel {

    private static final Logger logger = LogManager.getLogger(EducationModelMEL.class);
    ...
    public EducationModelMEL(DataContainerWithSchools dataContainer, Properties properties, Random rnd) {
    ...
    public Collection<EducationEvent> getEventsForCurrentYear(int year) {
        ...
        for(Person person: dataContainer.getHouseholdDataManager().getPersons()) {
            ...
            switch (occupation){
                ...
                case STUDENT:
                    int oldSchoolType = -1;
                    if(((PersonHealthMEL)person).getSchoolId()> 0){
                        oldSchoolType = ((DataContainerWithSchools)dataContainer).getSchoolData().getSchoolFromId(((PersonHealthMEL)person).getSchoolId()).getType();
                    }else if(((PersonHealthMEL)person).getSchoolId()== -2){ //TODO: what does -2 means? sp has no school id -2
                        oldSchoolType = SchoolDataImpl.guessSchoolType((PersonHealthMEL) person);
                    }else if(((PersonHealthMEL)person).getSchoolId()== -1){
                        studentMissSchoolId++;
                        int schoolType = SchoolDataImpl.guessSchoolType((PersonHealthMEL) person);
                        School school = findSchool(person, schoolType);
                        ((PersonHealthMEL) person).setSchoolId(school.getId());
                    }
    ...
    public boolean handleEvent(EducationEvent event) {
        PersonHealthMEL pp = (PersonHealthMEL) dataContainer.getHouseholdDataManager().getPersonFromId(event.getPersonId());
        if (pp != null) {
    ...
    boolean updateEducation(PersonHealthMEL person, School school) {
    ...
    boolean leaveSchoolToWork(PersonHealthMEL person) {
