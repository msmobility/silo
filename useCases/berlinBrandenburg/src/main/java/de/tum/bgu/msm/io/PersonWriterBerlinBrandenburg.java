...
public class PersonWriterBerlinBrandenburg implements PersonWriter {

    private final static Logger logger = LogManager.getLogger(PersonWriterBerlinBrandenburg.class);

    private final HouseholdDataManager householdData;

    public PersonWriterBerlinBrandenburg(HouseholdDataManager householdData) {
    ...
    public void writePersons(String path) {
        ...
        for (Person pp : householdData.getPersons()) {
            ...
            pwp.print(",");
            pwp.print(((PersonBerlinBrandenburg)pp).getNationality().toString());
            pwp.print(",");
            pwp.print(0);
            pwp.print(",");
            pwp.print(((PersonBerlinBrandenburg)pp).getSchoolId());
            pwp.println();
