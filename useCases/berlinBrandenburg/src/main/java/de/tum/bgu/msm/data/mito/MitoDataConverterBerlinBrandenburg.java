...
public class MitoDataConverterBerlinBrandenburg implements MitoDataConverter {

    private final static Logger logger = LogManager.getLogger(MitoDataConverterBerlinBrandenburg.class);
    ...
    private void convertZones(DataSet dataSet, DataContainer dataContainer) {
        for (Zone siloZone : dataContainer.getGeoData().getZones().values()) {
            MitoZone zone = new MitoZone(siloZone.getZoneId(), ((ZoneBerlinBrandenburg) siloZone).getAreaTypeSG());
        ...
        for (Household siloHousehold : dataContainer.getHouseholdDataManager().getHouseholds()) {
            ...
            if (siloHousehold.getPersons().values().stream().anyMatch(p -> p.getAge() >= 18)) {
                ...
                for (Person person : siloHousehold.getPersons().values()) {
                    MitoPerson mitoPerson = convertToMitoPp((PersonBerlinBrandenburg) person, household, dataSet, dataContainer);
    ...
    private MitoPerson convertToMitoPp(PersonBerlinBrandenburg person, MitoHousehold household, DataSet dataSet, DataContainer dataContainer) {
        ...
        switch (mitoOccupationStatus) {
            case WORKER:
                if (person.getJobId() > 0) {
                    JobBerlinBrandenburg job = (JobBerlinBrandenburg) dataContainer.getJobDataManager().getJobFromId(person.getJobId());
                    jobType = job.getType();
