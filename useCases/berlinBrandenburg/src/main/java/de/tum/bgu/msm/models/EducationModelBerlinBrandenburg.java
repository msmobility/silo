 ...
 **/
public class EducationModelBerlinBrandenburg extends AbstractModel implements EducationModel {

    private static final Logger logger = LogManager.getLogger(EducationModelBerlinBrandenburg.class);
    ...
    public EducationModelBerlinBrandenburg(DataContainerWithSchools dataContainer, Properties properties, Random rnd) {
            ...
            switch (occupation){
                ...
                case STUDENT:
                    ...
                    if(((PersonBerlinBrandenburg)person).getSchoolId()> 0){
                        oldSchoolType = ((DataContainerWithSchools)dataContainer).getSchoolData().getSchoolFromId(((PersonBerlinBrandenburg)person).getSchoolId()).getType();
    ...
    public boolean handleEvent(EducationEvent event) {
        PersonBerlinBrandenburg pp = (PersonBerlinBrandenburg) dataContainer.getHouseholdDataManager().getPersonFromId(event.getPersonId());
        if (pp != null) {
    ...
    boolean updateEducation(PersonBerlinBrandenburg person, School school) {
    ...
    boolean leaveSchoolToWork(PersonBerlinBrandenburg person) {
