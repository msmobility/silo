...
public class MarryDivorceTest {
    ...
    public void testModelOne() {
        Person person = dataContainer.getHouseholdData().createPerson(1, 20, Gender.MALE, Race.other, 1, -1, 0);
        Assert.assertEquals((0.05926 /2) * SCALE, calculator.calculateMarriageProbability(person), 0.);
    }

    @Test
    public void testModelTwo() {
        Person person = dataContainer.getHouseholdData().createPerson(1, 50, Gender.FEMALE, Race.other, 1, -1, 0);
