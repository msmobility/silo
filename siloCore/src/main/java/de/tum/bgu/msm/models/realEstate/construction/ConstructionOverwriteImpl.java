    ...
    private void addDwellings (int year) {
        ...
        for (Integer[] data: list) {
            ...
            if (restriction != 0) {
                ...
                price = (int) (Math.abs(restriction) * dataContainer.getHouseholdDataManager().getMedianIncome(msa) / 12 * 0.18 + 0.5);
            }
            Dwelling dd = factory.createDwelling(ddId, zoneId, null, -1, dataContainer.getRealEstateDataManager().getDwellingTypes().get(dto), size, quality, price, restriction, year);
