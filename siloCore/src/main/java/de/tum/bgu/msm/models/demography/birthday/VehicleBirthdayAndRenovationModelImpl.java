...
public class VehicleBirthdayAndRenovationModelImpl extends AbstractModel implements EventModel<VehicleBirthdayEvent> {
    ...
    private boolean checkVehicleBirthday(VehicleBirthdayEvent event) {
        ...
        if (age > 15){
            ...
            event.getHousehold().getVehicles().add(new Car(VehicleUtil.getHighestVehicleIdInHousehold(event.getHousehold()), ((Car) event.getVehicle()).getCarType(), VehicleUtil.getVehicleAgeInBaseYear()));
        }
