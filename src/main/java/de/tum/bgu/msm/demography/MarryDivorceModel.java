...
public class MarryDivorceModel {
    ...
    public void choosePlannedMarriage (int coupleId, MovesModel move, InOutMigration migration,
                                       AutoOwnershipModel aoModel, RealEstateDataManager realEstateData,
                                       Accessibility accessibility, HouseholdDataManager householdData,
...
//            }
        ...
        if (moveTo == 1) {
            ...
            hhOfPartner2.removePerson(partner2, householdData);
            hhOfPartner1.addAdultPerson(partner2);
            moveRemainingPersonsIfAllChildren(hhOfPartner2.getId(), hhOfPartner1, householdData);
            ...
            hhOfPartner1.removePerson(partner1, householdData);
            hhOfPartner2.addAdultPerson(partner1);
            moveRemainingPersonsIfAllChildren(hhOfPartner1.getId(), hhOfPartner2, householdData);
            if (partner1.getId() == SiloUtil.trackPp || partner2.getId() == SiloUtil.trackPp || hhOfPartner1.getId() == SiloUtil.trackHh ||
                    hhOfPartner2.getId() == SiloUtil.trackHh) SiloUtil.trackWriter.println("Person " + partner1.getId() +
            ...
            hhOfPartner1.removePerson(partner1, householdData);
            moveRemainingPersonsIfAllChildren(hhOfPartner1.getId(), newHh, householdData);
            hhOfPartner2.removePerson(partner2, householdData);
            moveRemainingPersonsIfAllChildren(hhOfPartner2.getId(), newHh, householdData);
            newHh.setType();
            newHh.setHouseholdRace();
            int newDwellingId = move.searchForNewDwelling(new Person[]{partner1,partner2}, accessibility);
            if (newDwellingId < 0) {
                migration.outMigrateHh(newHhId, true, householdData, jobData);
            ...
            move.moveHousehold(newHh, -1, newDwellingId, realEstateData);
        }
        partner1.setRole(PersonRole.married);
        partner2.setRole(PersonRole.married);
        aoModel.simulateAutoOwnership(Household.getHouseholdFromId(partner1.getHhId()), accessibility, jobData);
        EventManager.countEvent(EventTypes.checkMarriage);
    }


    private void moveRemainingPersonsIfAllChildren(int oldHhId, Household newHh, HouseholdDataManager householdData) {
        ...
        if (onlyChildren) {
            for (Person per: remainingPersons) {
                oldHh.removePerson(per, householdData);
    ...
    public void chooseDivorce (int perId, MovesModel move, AutoOwnershipModel aoModel,
        ...
        if (rnum < divorceProbability[per.getType().ordinal()]) {
            ...
            int newDwellingId = move.searchForNewDwelling(new Person[] {per}, accessibility);
            if (newDwellingId < 0) {
            ...
            move.moveHousehold(newHh, -1, newDwellingId, realEstateData);
            aoModel.simulateAutoOwnership(newHh, accessibility, jobData);
