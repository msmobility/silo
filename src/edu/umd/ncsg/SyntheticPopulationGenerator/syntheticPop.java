...
public class syntheticPop {
    ...
    protected static final String PROPERTIES_JOB_CONTROL_TOTAL       = "job.control.total";
    ...
    protected HashMap<String, Integer[]> jobTarget;
...
//        writeSyntheticPopulation();
    ...
    private void createJobs () {
        ...
        for (int zone: geoData.getZones()) {
            for (int jobTp = 0; jobTp < JobType.getNumberOfJobTypes(); jobTp++) {
                if (jobInventory[jobTp][0][zone] > 0) {
                    for (int i = 1; i <= jobInventory[jobTp][0][zone]; i++) {
    ...
    private void processPums() {
        ...
        new Accessibility(rb, SiloUtil.getBaseYear());                        // read in travel times and trip length frequency distribution
...
//                            if (hoursWorked > 34) fullTime[personCounter] = true;
    ...
    private void savePumsRecord (int pumaZone, int weight, int hhSize, int pumsDdType, int bedRooms,
        ...
        for (int count = 0; count < weight; count++) {
            ...
            if (newHhId == SiloUtil.trackHh || true) {
        ...
        for (int taz: geoData.getZones()) {
            ...
            for (DwellingType dt: DwellingType.values()) {
                ...
                while (vacDwellingsModel < SiloUtil.rounder(targetThisTypeThisZoneAbs,0)) {
                    ...
                    int newDdId = RealEstateDataManager.getNextDwellingId();
                    new Dwelling(newDdId, dd.getZone(), -1, dd.getType(), dd.getBedrooms(), dd.getQuality(),
...
//        for (Household hh: Household.getHouseholdArray()) {
//            int taz = Dwelling.getDwellingFromId(hh.getDwellingId()).getZone();
