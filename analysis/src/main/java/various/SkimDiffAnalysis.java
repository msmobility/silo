...
public class SkimDiffAnalysis {
    ...
    public static void main(String[] args) {
        ...
        DoubleMatrix2D matrix1 = skim1.getMatrixForMode(TransportMode.car);
        DoubleMatrix2D matrix2 = skim2.getMatrixForMode(TransportMode.car);

        DoubleMatrix2D diff = matrix1.assign(matrix2, (d1, d2) -> d1 - d2);
        ...
        for (SimpleFeature feature : allFeatures) {
            ...
            {
                final DoubleMatrix1D row = matrix1.viewRow(zoneId);
                final DoubleMatrix1D col = matrix1.viewColumn(zoneId);
            ...
            {
                final DoubleMatrix1D row = matrix2.viewRow(zoneId);
                final DoubleMatrix1D col = matrix2.viewColumn(zoneId);
                newFeature.setAttribute("avg_to_2", row.zSum() / row.size());
                newFeature.setAttribute("avg_from_2", col.zSum() / col.size());
            }

            {
                final DoubleMatrix1D row = diff.viewRow(zoneId);
                final DoubleMatrix1D col = diff.viewColumn(zoneId);
