package de.tum.bgu.msm.syntheticPopulationGenerator.lisbon.allocation;
...
import de.tum.bgu.msm.syntheticPopulationGenerator.lisbon.DataSetSynPopLIS;
import de.tum.bgu.msm.syntheticPopulationGenerator.lisbon.preparation.MicroDataManager;
...
public class GenerateHouseholdsPersonsDwellings {
    ...
    private void priceRegression(Dwelling dwell) {
        ...
        //job density
        int lsoaID = ((DataSetSynPopLIS)dataSetSynPop).getTazMunicipality().get(dwell.getZoneId());
        if(((DataSetSynPopLIS)dataSetSynPop).getLsoaAttributes().get(lsoaID).get("tot")>0){
            logPrice += 0.017 * Math.log(((DataSetSynPopLIS)dataSetSynPop).getLsoaAttributes().get(lsoaID).get("tot"));
        }
