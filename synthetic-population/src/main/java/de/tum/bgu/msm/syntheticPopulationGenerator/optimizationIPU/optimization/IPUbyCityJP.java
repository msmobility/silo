...
public class IPUbyCityJP {

    private static final Logger logger = Logger.getLogger(IPUbyCityJP.class);
    ...
    private List<Integer> householdsInRegion;
    ...
    public IPUbyCityJP(DataSetSynPop dataSetSynPop){
    ...
    public void summarizeErrorsAndWeights(int municipality, int iteration){
        ...
        double[] weights = new double[dataSetSynPop.getFrequencyMatrix().getRowCount()];
        int hh = 0;
        for (int i = 1; i < dataSetSynPop.getFrequencyMatrix().getRowCount(); i++){
            if (householdsInRegion.contains(i)){
                weights[i] = minWeightsByMun.get(municipality)[hh];
                hh++;
            } else {
                weights[i] = 0;
            }
        }
        dataSetSynPop.getWeights().appendColumn(weights, Integer.toString(municipality));
        for (String attribute : PropertiesSynPop.get().main.attributesMunicipality){
    ...
    public void initializeErrorsandTotals(int municipality){
        ...
        //select the households on where to calculate the weights
        int regionsforFrequencyMatrix = (int) PropertiesSynPop.get().main.zonalDataIPU.getIndexedValueAt(municipality, "V2");
        householdsInRegion = new ArrayList<>();
        for (int i = 1; i <= dataSetSynPop.getFrequencyMatrix().getRowCount(); i++) {
            if (dataSetSynPop.getDwellingTable().get(i, "PtResCode") == regionsforFrequencyMatrix) {
                householdsInRegion.add(i);
            }
        }
        int[] households = householdsInRegion.stream().mapToInt(Integer::intValue).toArray();


        //initialize errors
        //initialize errors, considering the first weight (equal to 1)
        for (String attribute : PropertiesSynPop.get().main.attributesMunicipality) {
            int[] values = new int[households.length];
            double[] dummy = SiloUtil.createArrayWithValue(households.length, 1.);
            double[] dummy1 = SiloUtil.createArrayWithValue(households.length, 1.);
            for (int i = 0; i <= households.length; i++) {
                values[i - 1] = (int) dataSetSynPop.getFrequencyMatrix().getIndexedValueAt(i, attribute);
            }
