...
public class CarOwnership {
    ...
    private Map<Integer, Float> autoByHhsizeByLSOACumProbabilities(Household hh){
        int lsoaID = ((DataSetSynPopLIS)dataSetSynPop).getTazMunicipality().get(dataContainer.getRealEstateDataManager().getDwelling(hh.getDwellingId()).getZoneId());
        int hhsize = Math.min(hh.getHhSize(),4);
        return ((DataSetSynPopLIS)dataSetSynPop).getCarOwnershipProbabilityByHhsizeAndLSOA().get(lsoaID).get(hhsize);
    }
