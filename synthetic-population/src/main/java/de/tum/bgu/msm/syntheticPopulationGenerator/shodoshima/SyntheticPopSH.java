...
import de.tum.bgu.msm.io.DwellingWriterTak;
...
public class SyntheticPopSH implements SyntheticPopI {
    ...
    public SyntheticPopSH(ResourceBundle rb, DataSetSynPop dataSetSynPop, Properties properties) {
        // Constructor
        this.rb = rb;
    ...
    public void runSP(){
        ...
        ExtractMicroDataSH extractMicroData = new ExtractMicroDataSH(rb, dataSetSynPop);
    ...
    private void readInputData() {
        ...
        marginalsMunicipality.buildIndex(marginalsMunicipality.getColumnPosition("id"));
        jobsMunicipality = SiloUtil.readCSVfile(rb.getString(PROPERTIES_JOBS_MUNICIPALITY));
        jobsMunicipality.buildIndex(jobsMunicipality.getColumnPosition("id"));
        ...
        for (int row = 1; row <= selectedMunicipalities.getRowCount(); row++){
                ...
                int city = (int) selectedMunicipalities.getValueAt(row,"id");
                municipalities.add(city);
                int county = (int) selectedMunicipalities.getValueAt(row,"id");
                ...
                if (selectedMunicipalities.getValueAt(row,"SelectAllocation") == 0){
                    municipalitiesWithZeroPopulation.add((int) selectedMunicipalities.getValueAt(row,"id"));
                }
        ...
        logger.info("   Starting to read distance matrix");
        ...
        for (int i = 1; i <= distanceMatrix.getRowCount(); i++){
            for (int j = 1; j <= distanceMatrix.getColumnCount(); j++){
                if (i == j) {
                    distanceMatrix.setValueAt(i,j, 0.4f);
                } else {
                    distanceMatrix.setValueAt(i,j, distanceMatrix.getValueAt(i,j));
                }
            }
        }
        ...
        regionsforFrequencyMatrix.buildIndex(regionsforFrequencyMatrix.getColumnPosition("region"));
    ...
    private void generateJobs(){
        ...
        for (int municipality = 0; municipality < cityID.length; municipality++) {
            ...
            rasterCellsMatrix.buildIndex(rasterCellsMatrix.getColumnPosition("id"));
...
*/
    ...
    private void generateHouseholdsPersonsDwellings(){
        ...
        for (int municipality = 0; municipality < cityID.length; municipality++){
            int municipalityID = cityID[municipality];
            if (dataSetSynPop.getMunicipalitiesWithZeroPopulation().contains(municipalityID)) {
                continue;
            }
            ...
            //int v2zone = (int) regionsforFrequencyMatrix.getIndexedValueAt(municipalityID, "region");
            int v2zone = 1;
            if (householdsForFrequencyMatrix.containsKey(v2zone)) {
                ...
                for (int i = 0; i < tazInCity.length; i++){
                    probTaz[i] = rasterCellsMatrix.getIndexedValueAt(tazInCity[i],"ppTotal");
                ...
                for (int row = 0; row < totalHouseholds; row++) {
                    ...
                    for (int rowPerson = 0; rowPerson < householdSize; rowPerson++) {
                        ...
                        hhPersons++;
                        counterMunicipality = updateCountersPerson(pers, counterMunicipality, municipalityID,ageBracketsPerson);
                    }
                ...
                for (int attribute = 1; attribute < attributesHouseholdIPU.length; attribute++){
                    float allocated = counterMunicipality.getIndexedValueAt(municipalityID,attributesHouseholdIPU[attribute]);
                    float controlled = marginalsMunicipality.getIndexedValueAt(municipalityID,attributesHouseholdIPU[attribute]);
                    if(controlled==0){
                        continue;
                    }
                    float error = Math.abs(( allocated-controlled) / controlled);
    ...
    private void summarizeByZone(int municipality, Map<Integer, Household> householdMap, Map<Integer, Person> personMap) {
        zonalSummary.get(municipality).put("hhTotal", (double) householdMap.values().stream().mapToInt(x -> x.getId()).count());
        zonalSummary.get(municipality).put("nHH_1", (double) householdMap.values().stream().filter(x ->x.getHhSize() == 1).count());
        zonalSummary.get(municipality).put("nHH_2", (double) householdMap.values().stream().filter(x ->x.getHhSize() == 2).count());
        zonalSummary.get(municipality).put("nHH_3", (double) householdMap.values().stream().filter(x ->x.getHhSize() == 3).count());
        zonalSummary.get(municipality).put("nHH_4", (double) householdMap.values().stream().filter(x ->x.getHhSize() == 4).count());
        zonalSummary.get(municipality).put("nHH_5", (double) householdMap.values().stream().filter(x ->x.getHhSize() >= 5).count());
        zonalSummary.get(municipality).put("ppTotal", (double) personMap.values().stream().mapToInt(x -> x.getId()).count());

        int[] ageBracketsGender = new int[]{5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80,85,120};
        ...
        for (int age : ageBracketsGender){
            ...
            zonalSummary.get(municipality).put("M_"+age, maleYoungerThan - countAgeMale);
            ...
            zonalSummary.get(municipality).put("F_"+age, femaleYoungerThan - countAgeFemale);
...
/*        } else if (incomeClass == 50) {
    ...
    public static TableDataSet updateCountersPerson (Person person, TableDataSet attributesCount,int mun, int[] ageBracketsPerson) {
        ...
        attributesCount.setIndexedValueAt(mun, "ppTotal", attributesCount.getIndexedValueAt(mun, "ppTotal") + 1);
