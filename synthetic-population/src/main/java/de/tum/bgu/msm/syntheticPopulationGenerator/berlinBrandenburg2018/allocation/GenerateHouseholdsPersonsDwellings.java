package de.tum.bgu.msm.syntheticPopulationGenerator.berlinBrandenburg2018.allocation;
...
import de.tum.bgu.msm.syntheticPopulationGenerator.berlinBrandenburg2018.preparation.MicroDataManager;
...
public class GenerateHouseholdsPersonsDwellings {
    ...
    private void generatePersons(int hhSelected, Household hh){
        ...
        for (int person = 0; person < hhSize; person++) {
            ...
            int income = microDataManager.translateIncome(dataSetSynPop.getPersonTable().get(personSelected, "netIncomeAverage"));
            ...
            PersonRole personRole = microDataManager.translatePersonRole(dataSetSynPop.getPersonTable().get(personSelected, "personRoleInFamily"));
    ...
    private void generateDwelling(int hhSelected, int idHousehold, int tazSelected, int municipality){
        ...
        int yearBracket = dataSetSynPop.getDwellingTable().get(hhSelected, "ddConstructionYear");
        int year = microDataManager.dwellingYearfromBracket(yearBracket);
        int floorSpace = dataSetSynPop.getDwellingTable().get(hhSelected, "ddArea");
        int useInteger = dataSetSynPop.getDwellingTable().get(hhSelected, "ddOwnership");
        DwellingUsage usage = DwellingUsage.valueOf(useInteger);
        int buildingSize = dataSetSynPop.getDwellingTable().get(hhSelected, "ddBuildingSize");
        int ddHeatingEnergyType = dataSetSynPop.getDwellingTable().get(hhSelected, "ddHeatingEnergyType");
        int districtHeatingType = dataSetSynPop.getDwellingTable().get(hhSelected, "ddDistrictHeating");
        int blockCentralHeatingType = dataSetSynPop.getDwellingTable().get(hhSelected, "ddBlockCentralHeating");
        int numberOfHeatingTypes = dataSetSynPop.getDwellingTable().get(hhSelected, "ddNumberOfHeatingTypes");
        int quality = microDataManager.guessDwellingQuality(districtHeatingType, blockCentralHeatingType, ddHeatingEnergyType, numberOfHeatingTypes, yearBracket);
